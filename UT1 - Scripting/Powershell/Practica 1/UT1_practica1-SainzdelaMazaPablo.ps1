...
function Write-Log {
    ...
    $logLine = "[$timestamp] $Level $computerName $SessionCode $Message"
...
if (-not (Test-Path $OutputPath)) {
    try {
        # Intentar crear la carpeta de salida
        Write-Host "La ruta de salida no existe. Intentando crearla..." 
        New-Item -ItemType Directory -Path $OutputPath -Force | Out-Null # Out Null elimina la salida por pantalla
        Write-Log -Level INFO -Message "Creando carpeta" -LogPath $LogPath -SessionCode $SessionCode
    }
    catch {
        # Si no se puede crear la carpeta en la red o directorio, mostrar advertencia
        Write-Warning "No se pudo crear la carpeta en la ruta especificada ($OutputPath). Usando la carpeta Documentos como ruta alternativa."
        
        # Establecer ruta alternativa en la carpeta Documentos del usuario
        $OutputPath = [System.IO.Path]::Combine($env:USERPROFILE, "Documents", "Practica_PS")
        
        # Intentar crear la ruta alternativa en Documentos
        if (-not (Test-Path $OutputPath)) {
...
function Test-WriteAccess {
    ...
    if (-not (Test-Path $LogPath)) {
        try {
            ...
            Write-Warning "No se pudo crear la carpeta de logs en $LogPath. Se omite el registro."
        }
    }

    $tempFile = Join-Path $Path "test_write_$(Get-Random).tmp"
    try {
        # Intentar crear un archivo temporal
        Set-Content -Path $tempFile -Value "test" -ErrorAction Stop
        
        # Si se pudo crear, eliminarlo
        Remove-Item -Path $tempFile -Force -ErrorAction Stop

        Write-Host "Comprobacion de escritura correcta en: $Path" -ForegroundColor Green
        return $true
    }
    catch {
        # Si falla, registrar el problema y cambiar la ruta a Documentos
        Write-Warning "No se puede escribir en $Path. Se usará una ruta local alternativa."
        
        $warningMessage = "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] No se pudo escribir en $Path. Se usará la ruta local."
        $logFile = Join-Path $LogPath "$SessionCode_log.log"
        Add-Content -Path $logFile -Value $warningMessage -ErrorAction SilentlyContinue
        
        # Cambiar a Documentos como ruta alternativa
        $OutputPath = [System.IO.Path]::Combine($env:USERPROFILE, "Document", "Practica_PS")
        
        if (-not (Test-Path $OutputPath)) {
            try {
                New-Item -ItemType Directory -Path $OutputPath -Force | Out-Null
                Write-Host "Ruta alternativa creada: $OutputPath" -ForegroundColor Yellow
            } catch {
                Write-Warning "No se pudo crear la ruta alternativa en Documentos. Verifica permisos."
                Write-Log -Level ERROR -Message "No se pudo crear la ruta alternativa en Documentos. Verifica permisos" -LogPath $LogPath -SessionCode $SessionCode
            }
        }
        return $false
    }
}
if (-not (Test-WriteAccess -Path $OutputPath -LogPath $LogPath)) {
    Write-Warning "Se ha cambiado la ruta de salida a la carpeta local de Documentos."
    Write-Log -Level WARN -Message "Se ha cambiado la ruta de salida a la carpeta local." -LogPath $LogPath -SessionCode $SessionCode
}
## Comprobacion de existencia de carpeta de logs
if (-not (Test-Path "$LogPath")) {
    New-Item -ItemType Directory -Path "$LogPath" -Force
}

$csv = Join-Path $OutputPath "$env:computername-$SessionCode.csv"
...
if ($checkInstalledApps -eq 1) {
  ...
  $appsList = "N/A"
  Write-Log -Level ERROR -Message "Installed apps check is disabled." -LogPath $LogPath -SessionCode $SessionCode
}
